...
const LoginRegister = () => {
  const [formType, setFormType] = useState(null); // Initially, no form is shown
    ...
    if (username && password && phone && shopName) {
      try {
        ...
        if (res.ok) {
          ...
          setFormType('login'); // After successful registration, show login form
        } else {
  ...
  if (!formType && !isLoggedIn) {
    return (
      <div className="auth-wrapper">
        <div className="container">
          <button onClick={() => setFormType('register')}>Sign Up</button>
